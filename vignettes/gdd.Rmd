...
- Case `"B"` - The heat units are calculated based on the difference between the mean daily temperature and the threshold (`tbase`). 
...
```{r}
df_plot2 <- pivot_longer(gdd_data, case_b:case_d)
p2 <- ggplot(df_plot2) +
  geom_line(aes(day, value, color = name))
p2
```
